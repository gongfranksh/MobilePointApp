    ...
    public void onCreate() {
        ...
        mClient.registerLocationListener(new BDLocationListener() {
            ...
            public void onReceiveLocation(BDLocation bdLocation) {
                if (MobilePointApplication.getAppContext().getLocations().size() >= 5) {
                    MobilePointApplication.getAppContext().getLocations().clear();//存放位置信息超出阈值之后清空位置列表
                }
                MobilePointApplication.getAppContext().getLocations().add(bdLocation);//往Application中添加地理位置
            }
        });
        super.onCreate();
    }
